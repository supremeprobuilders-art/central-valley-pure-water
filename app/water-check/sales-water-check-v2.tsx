...
function healthRatioLabel(health: HealthComparison): string {
  if (health.multiplier >= 1) return `${formatHealthMultiplier(health.multiplier)} the CA Public Health Goal`;
  return `${formatHealthMultiplier(health.multiplier)} of the CA Public Health Goal`;
}

function recommendationLabel(key: string): string {
  switch (key) {
    case "reverse-osmosis":
      return "Kitchen RO-focused Complete Home path";
    case "water-softener":
...
export function SalesWaterCheckV2() {
  ...
  const loadReport = useCallback(async (system: WaterSystem) => {
    ...
    try {
      ...
      setReport(data);
      const healthComparisons = data.analytes
        .map(healthFor)
      ...
      track("water_monitoring_report_view", {
        ...
        health_benchmark_comparisons: healthComparisons.length,
        highest_health_multiple: healthComparisons.length
          ? Math.max(...healthComparisons.map((comparison) => comparison.multiplier))
          : null,
  ...
  const analyzed = useMemo<AnalyteWithHealth[]>(() => {
    if (!report) return [];
    return report.analytes.map((analyte) => ({ analyte, health: healthFor(analyte) }));
  }, [report]);

  const primaryHealth = useMemo<AnalyteWithHealth | null>(() => {
    const candidates = analyzed
      ...
      .sort((a, b) => {
        ...
        return b.health.multiplier - a.health.multiplier;
      });
    return candidates[0] ?? null;
  }, [analyzed]);

  const primaryMcl = useMemo<ReportAnalyte | null>(() => {
  ...
  const topAnalytes = useMemo<AnalyteWithHealth[]>(() => {
    return [...analyzed]
      .sort((a, b) => {
        const healthA = a.health?.multiplier ?? -1;
        const healthB = b.health?.multiplier ?? -1;
  ...
  const smsHref = useMemo(() => {
    ...
    const lines = [
      ...
      `Water-report treatment path: ${quoteContext}`,
  ...
  }, [address, bathrooms, email, householdSize, name, phone, provider, quoteContext, tier, zip]);
    ...
    <section className={styles.funnel} aria-labelledby="water-check-tool-heading">
      ...
      {reportLoading ? (
        <div className={styles.loading} role="status">
          <i aria-hidden="true" />
          Calculating your water report from selected recent public monitoring records…
        </div>
      ) : null}
        ...
        <div className={styles.resultStack} ref={resultRef} tabIndex={-1}>
          ...
          <div className={styles.healthHero}>
            <div className={styles.healthHeroMain}>
              <span className={styles.healthEyebrow}>Largest health-based comparison in the selected results</span>
              {primaryHealth?.health ? (
                ...
                <>
                  ...
                  <small>No California PHG comparison was available for the selected detected analytes.</small>
                </>
              ) : (
                <>
                  <strong>—</strong>
                  <h3>No direct benchmark comparison available</h3>
                  <p>The selected records still appear below with their measured levels and context.</p>
                </>
              )}
            </div>
            ...
            <div className={styles.healthHeroStats}>
              <article>
                ...
                <span>detected analytes in selected recent records</span>
              </article>
              <article>
              ...
              <article>
                <strong>{formatDate(report.summary.latestSampleDate)}</strong>
                <span>latest selected sample date</span>
              </article>
            </div>
          </div>
          ...
          <div className={styles.benchmarkDisclosure}>
            ...
            <p>
              It is actual arithmetic: the highest selected monitoring result divided by the California OEHHA Public Health Goal for that chemical, when a compatible PHG is available. A PHG is a health-based goal, not an enforceable legal limit and not a line between “safe” and “dangerous.” The separate MCL is the regulatory standard. This report is not a sample from your home.
            </p>
          ...
          <div className={styles.detectedList}>
            ...
            <div className={styles.healthRows}>
              {topAnalytes.map(({ analyte, health }) => (
                <article key={analyte.id}>
                  ...
                  <div className={styles.healthRowMetric}>
                    {health ? (
                      <>
                        <strong>{formatHealthMultiplier(health.multiplier)}</strong>
                        <b>{healthRatioLabel(health)}</b>
                        <small>
                          {formatNumber(health.sampleValue)} {health.unit} ÷ {formatBenchmarkValue(health.benchmarkInSampleUnit)} {health.unit}
                        </small>
                      </>
                      ...
                      <>
                        ...
                        <b>No direct health-benchmark ratio</b>
                        <small>No compatible PHG or listed MCL comparison available.</small>
                      </>
                    )}
                  </div>
                  <div className={styles.healthRowLegal}>
            ...
            <details className={styles.details}>
              <summary>See source details and report limitations <span aria-hidden="true">+</span></summary>
              <div>
                <p>
                  We reviewed {report.summary.recordsReviewed.toLocaleString()} selected records across {report.summary.pointsReviewed} representative sampling points over the last {report.lookback.months} months. The ratio uses the highest selected result, not a utility-wide average.
                </p>
                <ul>
      ...
      {privateWell ? (
        <div className={styles.wellResult} ref={resultRef} tabIndex={-1}>
          ...
          <p>
            Private-well conditions can vary property by property. We can still price the Standard Complete Home package, but treatment recommendations beyond that start with property-specific water information.
          </p>
        </div>
      ) : null}
        ...
        <section className={styles.priceSection} aria-labelledby="package-price-heading">
          <div className={styles.priceHeader}>
            <div>
              ...
              <p>
                The report identifies the treatment path. Household size, bathrooms, peak demand, incoming water conditions and installation details determine final equipment sizing.
              </p>
            </div>
          ...
          <form className={styles.quoteForm} onSubmit={buildQuote}>
            <div className={styles.formHeading}>
              ...
              <div>
                ...
                <h3>Two home details help us size the quote path.</h3>
                <p>Standard, Plus and Dual Tank are not automatically assigned from ZIP alone.</p>
              </div>
            </div>
            ...
            <button className={styles.quoteButton} type="submit">Show my quote path <span aria-hidden="true">→</span></button>
